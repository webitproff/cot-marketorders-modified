...
if ($m == 'download') {
    ...
    if (file_exists($file) && ($marketorder['order_status'] == 'paid' || $marketorder['order_status'] == 'done') || $usr['isadmin'] || $usr['id'] == $marketorder['order_seller']) {
       /* === Hook === */
        foreach (cot_getextplugins('marketorders.download') as $pl) {
            include $pl;
        }
        /* ===== */
...
if ($m == 'getfile') {
    $prd = cot_import("prd", "G", "INT");
    $item = $db->query("SELECT * FROM $db_market WHERE item_id=" . $prd . " LIMIT 1")->fetch();
    if ($item["item_cost"] == "0.00") {
        $file = $cfg['plugin']['marketorders']['filepath'] . '/' . $item['item_file'];
